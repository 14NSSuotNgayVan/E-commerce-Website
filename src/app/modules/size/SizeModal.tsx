...
function SizeModal(props: any) {
    ...
    const handleSubmit: any = async () => {
        try {
            setPageLoading(true);
            const data = await (!formik.values?.id ? addSize(formik.values) : updateSize(formik.values));
                    ...
                    <Row className="p-2">
                        <Col sm={12}>
                            <TextValidator
                                ...
                                lable={"Tên kích cỡ"}
                                name="tenKichThuoc"
                                value={formik.values?.tenKichThuoc || ""}
                                type="text"
                                errors={formik.errors?.tenKichThuoc}
                                touched={formik.touched?.tenKichThuoc}
