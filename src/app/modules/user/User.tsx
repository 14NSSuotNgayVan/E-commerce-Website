...
import { deleteUser, searchUser } from "./services/UserServices";
...
function User() {
    ...
    const handleShowModal = (item: any = null) => {
        setIsShowModal(true);
        setItem(item);
    };
    ...
    const updatePageData = async (searchObject: any = {}) => {
        try {
            setPageLoading(true);
            const { data } = await searchUser(filterObject(searchObject));
            setTotalPage(data?.data?.totalPages);
            setTotalElements(data?.data?.totalElements);
            setNumberOfElements(data?.data?.numberOfElements);

            if (data?.code === RESPONSE_STATUS_CODE.SUCCESS) {
                setDataUser(data?.data?.content);
            }
        } catch (error) {
            ...
            <div className="bg-white mt-2 p-2">
                ...
                <div className="table-custom">
                    <TableCustom
                        ...
                        noToolbar={false}
                        buttonAdd={true}
                        handleOpenDialog={(e: any) => handleShowModal()}
                    />
                </div>
            </div>
            {isShowModal && <UserModal isShowModal={isShowModal} handleCloseModal={handleCloseModal} handleSearch ={handleSearch} item={item} />}
        </div>
    );
}
