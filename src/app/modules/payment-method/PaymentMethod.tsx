...
import { deletePaymentMethod, searchPaymentMethod } from "./services/PaymentMethodServices";
import TableCustom from "../component/table-custom/TableCustom";

function PaymentMethod() {
    const intl = useIntl();
    const { lang } = useMultiLanguage();
    const title = "GENERAL.PAYMENT_METHOD";
    ...
    const updatePageData = async (searchObject: any = {}) => {
        try {
            setPageLoading(true);
            const { data } = await searchPaymentMethod(filterObject(searchObject));
    ...
    const handleDeleteRows = async (ids: string) => {
        try {
            setPageLoading(true);
            const { data } = await deletePaymentMethod(ids);
            if (data?.code === RESPONSE_STATUS_CODE.SUCCESS) {
    ...
    return (
        <div className="spaces my-40">
            ...
            <PaymentMethodModal isShowModal={isShowModal} handleCloseModal={handleCloseModal} handleSearch={handleSearch} item={item} />
        </div>
    );
}
