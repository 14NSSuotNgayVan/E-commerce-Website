...
import { deleteColor, searchColor } from "./services/ColorServices";
import TableCustom from "../component/table-custom/TableCustom";

function Color() {
    const intl = useIntl();
    const { lang } = useMultiLanguage();
    const title = "GENERAL.COLOR";
    ...
    const handleDeleteRows = async (ids: string) => {
        try {
            setPageLoading(true);
            const { data } = await deleteColor(ids);
            if (data?.code === RESPONSE_STATUS_CODE.SUCCESS) {
    ...
    return (
        <div className="spaces my-40">
            ...
            <ColorModal isShowModal={isShowModal} handleCloseModal={handleCloseModal} handleSearch={handleSearch} item={item} />
        </div>
    );
}
