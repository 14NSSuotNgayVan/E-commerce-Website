import React, { useContext, useEffect, useState } from "react";
import { useFormik } from "formik";
import * as Yup from "yup";
import { Button, Col, Form, Modal, Row } from "react-bootstrap";
import { toast } from "react-toastify";
import useMultiLanguage from "~/app/hook/useMultiLanguage";
import AppContext from "~/app/AppContext";
import { initRole } from "./consts/RoleConst";
import TextValidator from "../component/input-field/TextValidator";
import { REGEX, RESPONSE_STATUS_CODE } from "../utils/Constant";
import { addRole, updateRole } from "./services/RoleServices";
import Autocomplete from "../component/input-field/Autocomplete";
import { heightSelectMutil } from "../component/input-field/StyleComponent";
import { INITIAL_SEARCH_OBJECT } from "../constant";
import { searchAuthority } from "../authority/services/AuthorityServices";


function RoleModal(props: any) {
    const {
        isShowModal,
        handleCloseModal,
        item,
        handleSearch
    } = props;
    const { lang } = useMultiLanguage();
    const { setPageLoading } = useContext(AppContext);

    const validationSchema = Yup.object().shape({
        name: Yup.string()
            .matches(REGEX.CHARACTER255, lang("VALIDATION.MAX255"))
            .required(lang("VALIDATION.REQUIRE"))
            .nullable(),
        authorities: Yup.array()
            .nullable(),
    });

    const handleSubmit: any = async () => {
        try {
            setPageLoading(true);
            const data = await (!formik.values?.id ? addRole(formik.values) : updateRole(formik.values));

            if (data?.data?.code === RESPONSE_STATUS_CODE.SUCCESS) {
                toast.success("Thành công!");
                handleClose();
                handleSearch();
                return;
            }
            toast.error(data?.data?.message);
        } catch {
            toast.error("Không thành công!");
        } finally {
            setPageLoading(false);
        }
    }

    const formik = useFormik({
        initialValues: initRole,
        validationSchema: validationSchema,
        onSubmit: handleSubmit,
    });

    const handleSelect = (name: string, value: any) => {
        formik.setFieldValue(name, value)
    }

    const handleClose = () => {
        formik.resetForm();
        handleCloseModal();
    }

    useEffect(() => {
        item && formik.setValues({ ...formik.values, ...item });
    }, [item]);

    return (
        <Modal
            show={isShowModal}
            size="lg"
            centered
            aria-labelledby="example-custom-modal-styling-title"
            onHide={handleClose}
        >
            <Form onSubmit={formik.handleSubmit}>
                <Modal.Header closeButton>
                    <Modal.Title
                        id="example-custom-modal-styling-title"
                        className="heading-5"
                    >
                        {lang(`${formik.values?.id ? 'GENERAL.UPDATE' : 'GENERAL.ADD'}`)}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body className="p-4">
                    <Row className="p-2">
                        <Col sm={6}>
                            <TextValidator
                                className="flex-row min-w-80"
                                isRequired
                                lable={"Vai trò"}
                                name="name"
                                value={formik.values?.name || ""}
                                type="text"
                                errors={formik.errors?.name}
                                touched={formik.touched?.name}
                                onChange={formik.handleChange}
                            />
                        </Col>
                        <Col sm={6}>
                            <Autocomplete
                                className="spaces z-index-8 width-100"
                                lable={"Quyền hạn : "}
                                options={[]}
                                searchFunction={searchAuthority}
                                searchObject={INITIAL_SEARCH_OBJECT}
                                value={formik.values?.authorities}
                                name={"authorities"}
                                onChange={(selectedOption) => handleSelect("authorities", selectedOption)}
                                isMulti={true}
                                closeMenuOnSelect={false}
                                getOptionValue={(option) => option?.name}
                                styles={heightSelectMutil("auto", "36px", "100px")}
                            />
                        </Col>
                    </Row>
                </Modal.Body>
                <Modal.Footer className="flex-center">
                    <button type="button" className="btn btn-secondary" onClick={handleClose}>{lang("BTN.CANCEL")}</button>
                    <button type="submit" className="btn btn-primary">{lang("BTN.SAVE")}</button>
                </Modal.Footer>
            </Form>
        </Modal>
    );
}
export { RoleModal };