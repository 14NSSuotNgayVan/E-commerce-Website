...
import { deleteRole, searchRole } from "./services/RoleServices";
import TableCustom from "../component/table-custom/TableCustom";

function Role() {
    const intl = useIntl();
    const { lang } = useMultiLanguage();
    const title = "GENERAL.ROLE";
    ...
    const ColumnsRole = [
        ...
        },
        {
            ...
            render: (row: any, index: number, numericalOrder: number, itemList: any) => <span>{row.authorities.map((item:any)=>item.name).join(', ')}</span>,
        },
    ];
    ...
    const updatePageData = async (searchObject: any = {}) => {
        try {
            setPageLoading(true);
            const { data } = await searchRole(filterObject(searchObject));
            setTotalPage(data?.data?.totalPages);
            setTotalElements(data?.data?.totalElements);
            setNumberOfElements(data?.data?.numberOfElements);

            if (data?.code === RESPONSE_STATUS_CODE.SUCCESS) {
                setDataRole(data?.data?.content);
            }
        } catch (error) {
    ...
    return (
        <div className="spaces my-40">
            ...
            <RoleModal isShowModal={isShowModal} handleCloseModal={handleCloseModal} handleSearch={handleSearch} item={item} />
        </div>
    );
}
